...
export default function IeIncomeList() {
  ...
  const load = useCallback(() => {
    ...
    Promise.all([apiJson(`/api/admin/income-expense/income-list?${sp.toString()}`), apiJson(`/api/admin/income-expense/overview?date=${today}`)])
      .then(([incomeData, overview]) => {
        setData(incomeData);
      ...
      <div className={`${sectionCard} p-4 md:p-5`}>
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
          <label className="block text-sm">
            ...
            <select className={inputCls} value={filters.store_id} onChange={(e) => setSp(updateSearchParams(sp, { store_id: e.target.value }))}>
          ...
          <label className="block text-sm">
            ...
            <input
              className={inputCls}
              placeholder="POS_Sale, Return_Refund..."
              value={filters.source_type}
              onChange={(e) => setSp(updateSearchParams(sp, { source_type: e.target.value }))}
            />
          </label>
          <label className="block text-sm">
          ...
          <div className="flex items-end gap-3">
            <button type="button" className="rounded-xl border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-700" onClick={() => setSp({})}>
              Clear filters
            </button>
          </div>
        </div>
              ...
              {(data.income ?? []).map((e) => (
                <tr key={e.id} className="border-t border-slate-100 align-top">
                  <td className="px-4 py-3 text-slate-700">
                    <div>{fmtDateOnly(e.income_date)}</div>
                    <div className="text-xs text-slate-400">{e.income_time || "—"}</div>
                  </td>
                  <td className="px-4 py-3 text-slate-700">{e.store_name || "Shared / unassigned"}</td>
                  <td className="px-4 py-3 text-slate-700">{e.source_type || "Income"}</td>
                  <td className="px-4 py-3">
                    <div className="font-medium text-slate-900">{e.description || "—"}</div>
                    {e.payment_method ? <div className="mt-1 text-xs text-slate-500">Payment: {e.payment_method}</div> : null}
                  </td>
                  <td className="px-4 py-3 text-slate-600">{e.reference_number || "—"}</td>
                  <td className={`px-4 py-3 text-right font-semibold ${Number(e.amount) < 0 ? "text-red-700" : "text-emerald-700"}`}>
                    {fmtMoney(e.amount)}
                  </td>
                </tr>
              ))}
